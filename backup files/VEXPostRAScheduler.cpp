...
#define DEBUG_TYPE "VEX-post-ra-scheduler"
...
namespace {
  class VEXPostRAScheduler : public MachineFunctionPass {
    const TargetInstrInfo *TII;
    RegisterClassInfo RegClassInfo;

  public:
    static char ID;
    VEXPostRAScheduler() : MachineFunctionPass(ID) {}
  ...
  char VEXPostRAScheduler::ID = 0;

  class VEXSchedulePostRA : public ScheduleDAGInstrs {
...
VEXSchedulePostRA::VEXSchedulePostRA(
  MachineFunction &MF, MachineLoopInfo &MLI,
    ...
    LiveRegs(TRI->getNumRegs()), EndIndex(0)
{
  ...
  const InstrItineraryData *InstrItins = TM.getSubtargetImpl(*MF.getFunction())->getInstrItineraryData();
  HazardRec =
    TM.getSubtargetImpl(*MF.getFunction())->getInstrInfo()->CreateTargetPostRAHazardRecognizer(InstrItins, this);
...
void VEXSchedulePostRA::enterRegion(MachineBasicBlock *bb,
                 MachineBasicBlock::iterator begin,
                 MachineBasicBlock::iterator end,
                 unsigned endcount) {
...
void VEXSchedulePostRA::exitRegion() {
  DEBUG({
      dbgs() << "*** Final schedule ***\n";
      dumpSchedule();
      dbgs() << '\n';
...
void VEXSchedulePostRA::dumpSchedule() const {
...
bool VEXPostRAScheduler::runOnMachineFunction(MachineFunction &Fn) {
  ...
  VEXSchedulePostRA Scheduler(Fn, MLI, AA, RegClassInfo, AntiDepMode,
                                 CriticalPathRCs);
...
/// this block.
///
void VEXSchedulePostRA::startBlock(MachineBasicBlock *BB) {
  // Call the superclass.
...
/// Schedule - Schedule the instruction range using list scheduling.
///
void VEXSchedulePostRA::schedule() {
  ...
  DEBUG(dbgs() << "********** VEX List Scheduling **********\n");
...
/// instruction, which will not be scheduled.
///
void VEXSchedulePostRA::Observe(MachineInstr *MI, unsigned Count) {
  if (AntiDepBreak != NULL)
...
/// FinishBlock - Clean up register live-range state.
///
void VEXSchedulePostRA::finishBlock() {
  if (AntiDepBreak != NULL)
...
///
void VEXSchedulePostRA::StartBlockForKills(MachineBasicBlock *BB) {
...
bool VEXSchedulePostRA::ToggleKillFlag(MachineInstr *MI,
                                          MachineOperand &MO) {
  // Setting kill flag...
  if (!MO.isKill()) {
    MO.setIsKill(true);
...
void VEXSchedulePostRA::FixupLoads(MachineBasicBlock *MBB) {
...
///
void VEXSchedulePostRA::FixupKills(MachineBasicBlock *MBB) {
...
void VEXSchedulePostRA::ReleaseSucc(SUnit *SU, SDep *SuccEdge) {
...
void VEXSchedulePostRA::ReleaseSuccessors(SUnit *SU) {
...
/// the Available queue.
void VEXSchedulePostRA::ScheduleNodeTopDown(SUnit *SU, unsigned CurCycle) {
...
/// schedulers.
void VEXSchedulePostRA::ListScheduleTopDown() {
  unsigned CurCycle = 0;
...
void VEXSchedulePostRA::EmitSchedule() {
...
FunctionPass *llvm::createVEXPostRAScheduler() {
    return new VEXPostRAScheduler();
}
