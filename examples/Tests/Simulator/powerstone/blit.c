#include <stdint.h>

uint32_t src[1024] =
{
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x12345678, 0x12345678, 0x12345678, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x12345678, 0x12345678, 0x12345678, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x12345678, 0x12345678, 0x12345678, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x12345678, 0x12345678, 0x12345678, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x12345678, 0x12345678, 0x12345678, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x10101010, 0x12345678, 0x10101010, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x10101010, 0x12345678, 0x10101010, 0x12345678,
  0x00005678, 0x12340000, 0x02040608, 0x00000001,
  0x10101010, 0x12345678, 0x10101010, 0x12345678,
  0
};
uint32_t dst[1024];
void blit(uint32_t saddr, uint32_t daddr, uint32_t n)
{
  int32_t d1, d2;
  uint32_t *s, *d, x, y;
  uint32_t soff, doff;
  s = (uint32_t *) ((uint32_t) src + (saddr >> 5));
  d = (uint32_t *) ((uint32_t) dst + (daddr >> 5));
  soff = saddr & 0x1f;
  doff = daddr & 0x1f;
  if (soff > doff)
    {
      d1 = soff - doff;
      d2 = 32 - d1;
      y = *d & (0xffffffff << (32 - doff));
      x = *s++ & ((uint32_t) 0xffffffff >> soff);
      y |= x << d1;
      for (; n >= 32; n -= 32, s++, d++)
	{
	  x = *s;
	  *d = y | (x >> d2);
	  y = x << d1;
	}
      x = *d & (0xffffffff << (32 - n + d1));
      *d = y | x;
    } 
  else if (soff < doff)
    {
      d1 = doff - soff;
      d2 = 32 - d1;
      y = *d & (0xffffffff << (32 - doff));
      x = *s++ & ((uint32_t) 0xffffffff >> soff);
      for (; n >= 32; n -= 32, s++, d++)
	{
	  *d = y | (x >> d1);
	  y = x << d2;
	  x = *s;
	}
      x = *d & (0xffffffff << (32 - n + d1));
      *d = y | (x >> d1);
    } 
  else
    {
      if (soff)
	{
	  y = *d & (0xffffffff << (32 - doff));
	  x = *s++ & ((uint32_t) 0xffffffff >> soff);
	  *d++ = x | y;
	  n -= (32 - soff);
	}
      for (; n >= 32; n -= 32, s++, d++)
	*d = *s;
      if (n)
	{
	  y = *d & ((uint32_t) 0xffffffff >> doff);
	  x = *s & (0xffffffff << (32 - soff));
	  *d = x | y;
	}
    }
}
int main()
{
  blit(17, 29, 1000 * 32);
  blit(29, 17, 1000 * 32);
  if (dst[0] != 291 || dst[4] != 1164411171 || dst[10] != 1080066048)
    {
      return 0x0F;
    } 
  else
    {
      return 0x42;
    }
}
