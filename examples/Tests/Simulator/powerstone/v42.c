...
uint32_t baddr, eaddr;
int32_t codedone;
uint8_t codebuf[8 * 1024];
uint8_t outbuf[8 * 1024];
uint8_t *eout = outbuf;
static uint8_t *gP, *gE;
int32_t getdata()
{
   if (gP >= gE)
   {
...
void putcode(uint32_t code, int32_t size)
{
   uint8_t *p;
   if (code == (-5))
   {
...
uint32_t search_dict(uint32_t string, uint8_t data)
{
   uint32_t kid;
   if (!string)
      return (data + 3);
...
void add_dict(uint32_t string, uint8_t data)
{
   struct dict *dp;
   uint32_t kid, prev, me;
   if (!string)
      return;
   dp = &dict[c1];
   dp->data = data;
...
void decode()
{
   ...
   for (;;)
   {
      ...
      {
      case (-2):
         // decode: error code in input stream
         return;
      case (-6):
         return;
      case (-1):
      ...
      } else
      {
         if (code == escape)
         {
            ...
            case 1:
               break;
            default:
               // decode: escape error input stream
               continue;
            }
            escape = (escape + 51) % 256;
         }
