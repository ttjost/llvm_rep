...
static int32_t bitsleft = 0;
static uint32_t *nextlong = huffbits;
...
int32_t getbit()
{
  ...
  if (--bitsleft < 0)
    {
      if (nextlong >= &huffbits[sizeof(huffbits)/sizeof(huffbits[0])])
	lastlong = 0;
      else
	{
	  lastlong = *nextlong++;
...
uint8_t val_dc_lum[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
			      0x07, 0x08, 0x09, 0x0A, 0x0B};
uint8_t val_ac_lum[] = {0x01, 0x02, 0x03, 0x00, 0x04, 0x11, 0x05, 0x12, 0x21,
...
void huff_dc_dec(int32_t *retval)
{
  int32_t i, s, l, p, code;
...
void huff_ac_dec(int16_t *data)
{
  int32_t i, j, icnt, ns, n, s, l, p, code, dindex;
  int32_t temp, data_zz[64];
...
void fast_idct_8(int16_t *in, int32_t stride)
{
  int32_t tmp10, tmp11, tmp12, tmp13;
  int32_t tmp20, tmp21, tmp22, tmp23;
  int32_t tmp30, tmp31;
  int32_t tmp40, tmp41, tmp42, tmp43;
  int32_t tmp50, tmp51, tmp52, tmp53;
  int32_t in0, in1, in2, in3, in4, in5, in6, in7;
  int32_t i, j;
  in0 = in[0];
  in1 = in[stride];
  in2 = in[stride * 2];
...
uint8_t qtbl_lum[] = {16, 11, 10, 16, 24, 40, 51, 61,
...
static int16_t dct_data[240 * 160];
