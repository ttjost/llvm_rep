//===-- HMCMCInstLower.cpp - Convert HMC MachineInstr to MCInst ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains code to lower HMC MachineInstrs to their corresponding
// MCInst records.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "HMC-mcinslower"

using namespace llvm;

HMCMCInstLower::HMCMCInstLower(HMCAsmPrinter &asmprinter)
: AsmPrinter(asmprinter) {
    DEBUG(errs() << "MCInstLower\n");
}

void HMCMCInstLower::Initialize(MCContext *C){
    Ctx = C;
}
...
// LowerOperand
MCOperand HMCMCInstLower::LowerOperand(const MachineOperand &MO,
...
void HMCMCInstLower::Lower(const MachineInstr *MI,
                           MCInst &OutMI,
                           MCInst &InBundleMI,
                           bool isInsideBundle,
    ...
    if (!MI) {
        InBundleMI.setOpcode(HMC::NOPInstr0 + numValArgumentOrLane);
        MCOperand MCOp1 = MCOperand::createReg(HMC::Reg0);
        InBundleMI.addOperand(MCOp1);
        MCOperand MCOp2 = MCOperand::createReg(HMC::Reg0);
