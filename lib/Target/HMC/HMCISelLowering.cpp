//===-- HMCISelLowering.cpp - HMC DAG Lowering Implementation -----------===//
//
...
#define DEBUG_TYPE "HMC-lower"

extern cl::opt<bool> EnableSPMs;

#if 0
SDValue HMCTargetLowering::getTargetNode(ConstantPoolSDNode *N, EVT Ty,
                                          SelectionDAG &DAG,
                                          unsigned Flag) const {
...
const char *HMCTargetLowering::getTargetNodeName(unsigned Opcode) const {

    switch (Opcode) {

        case HMCISD::WRAPPER:       return "HMCISD::WRAPPER";
        case HMCISD::PSEUDO_RET:    return "HMCISD::PSEUDO_RET";
        case HMCISD::PSEUDO_CALL:   return "HMCISD::PSEUDO_CALL";
        case HMCISD::PSEUDO_TAILCALL:   return "HMCISD::PSEUDO_TAILCALL";


        case HMCISD::MAX:           return "HMCISD::MAX";
        case HMCISD::MAXU:          return "HMCISD::MAXU";
        case HMCISD::MIN:           return "HMCISD::MIN";
        case HMCISD::MINU:          return "HMCISD::MINU";

        case HMCISD::ADDCG:         return "HMCISD::ADDCG";
        case HMCISD::DIVS:          return "HMCISD::DIVS";

        case HMCISD::ORC:           return "HMCISD::ORC";
        case HMCISD::SH1ADD:        return "HMCISD::SH1ADD";
        case HMCISD::SLCT:          return "HMCISD::SLCT";

        case HMCISD::MPYLL:         return "HMCISD::MPYLL";
        case HMCISD::MPYLLU:        return "HMCISD::MPYLLU";
        case HMCISD::MPYLH:         return "HMCISD::MPYLH";
        case HMCISD::MPYLHU:        return "HMCISD::MPYLHU";
        case HMCISD::MPYHH:         return "HMCISD::MPYHH";
        case HMCISD::MPYHHU:        return "HMCISD::MPYHHU";
        case HMCISD::MPYL:          return "HMCISD::MPYL";
        case HMCISD::MPYLU:         return "HMCISD::MPYLU";
...
//@HMCTargetLowering
HMCTargetLowering::HMCTargetLowering(const TargetMachine &TM,
...
: TargetLowering(TM), Subtarget(STI),
    ...
    addRegisterClass(MVT::i32, &HMC::GPRegsRegClass);
...
//    setOperationAction(ISD::DYNAMIC_STACKALLOC, MVT::i32,  Expand);
    ...
    setStackPointerRegisterToSaveRestore(HMC::Reg1);
    //setStackPointerRegisterToSaveRestore(HMC::Reg1);
...
SDValue HMCTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
...
HMCTargetLowering::getConstraintType(const std::string &Constraint) const {
...
HMCTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
    ...
    if (Constraint.size() == 1) {
        ...
        switch (Constraint[0]) {
            ...
            case 'r':   // GENERAL_REGS
                if (VT == MVT::i32)
                    return std::make_pair(0U, &HMC::GPRegsRegClass);
                return std::make_pair(0U, &HMC::BrRegsRegClass);
        }
    }
...
bool HMCTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
    // The HMC Target isn't yet aware of offsets.
    return false;
}
...
static void AnalyzeRetResult(CCState &State,
    ...
    State.AnalyzeCallResult(Ins, RetCC_HMC);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs){
    State.AnalyzeReturn(Outs, RetCC_HMC);
}
...
bool HMCTargetLowering::IsEligibleForTailCallOptimization(SDValue Callee,
...
SDValue
HMCTargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
    const HMCRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue
HMCTargetLowering::LowerFRAMEADDR(SDValue Op, SelectionDAG &DAG) const {
    const HMCRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue
HMCTargetLowering::LowerCall(CallLoweringInfo &CLI,
    ...
    HMCFunctionInfo *FuncInfo = MF.getInfo<HMCFunctionInfo>();
//    bool IsTailCall = false;
    ...
    auto *TFL = static_cast<const HMCFrameLowering *>(Subtarget.getFrameLowering());

    // If need, arguments on stack should be place after ScratchPad Area
    //ArgCCInfo.AllocateStack(TFL->getScratchArea(), 1);

    ArgCCInfo.AnalyzeCallOperands(Outs, CC_HMC_Address);
...
//        llvm_unreachable("Target does not yet support Tail Calls.");
    ...
    SDValue StackPtr =
            DAG.getCopyFromReg(Chain, DL, HMC::Reg1, getPointerTy(DLayout));;
    ...
    if (auto *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
        ...
        Callee = DAG.getNode(HMCISD::WRAPPER, DL, PtrVT, Callee);
...
//    // Glue the call to the argument copies, if any.
    ...
    if (IsTailCall) {
        ...
        return DAG.getNode(HMCISD::PSEUDO_TAILCALL, DL, NodeTys, Ops);
    }

    Chain = DAG.getNode(HMCISD::PSEUDO_CALL, DL, NodeTys, Ops);
    Glue = Chain.getValue(1);
...
void HMCTargetLowering::passByValArg(SDValue Chain, SDLoc DL,
                                      SmallVector<std::pair<unsigned, SDValue>, 10> RegsToPass,
    ...
    if (NumRegs) {
        const ArrayRef<MCPhysReg> ArgRegs = { HMC::Reg3, HMC::Reg4, HMC::Reg5, HMC::Reg6, HMC::Reg7, HMC::Reg8, HMC::Reg9, HMC::Reg10 };
...
void HMCTargetLowering::copyByValRegs(SDValue Chain, SDLoc DL, std::vector<SDValue> &OutChains, SelectionDAG &DAG,
    ...
    int FrameObjOffset;
    ArrayRef<MCPhysReg> ByValArgRegs = { HMC::Reg3, HMC::Reg4, HMC::Reg5, HMC::Reg6, HMC::Reg7, HMC::Reg8, HMC::Reg9, HMC::Reg10 };
...
void HMCTargetLowering::writeVarArgRegs(std::vector<SDValue> &OutChains,
...
void HMCTargetLowering::HandleByVal(CCState *State, unsigned &Size,
    ...
    if (State->getCallingConv() != CallingConv::Fast) {
        unsigned RegSizeInBytes = 4;
        const ArrayRef<MCPhysReg> IntArgRegs = { HMC::Reg3, HMC::Reg4, HMC::Reg5, HMC::Reg6, HMC::Reg7, HMC::Reg8, HMC::Reg9, HMC::Reg10 };
...
//
SDValue
HMCTargetLowering::LowerFormalArguments(SDValue Chain,
...
//        llvm_unreachable("Variable number of arguments not yet implemented!");
    ...
    CCInfo.AnalyzeFormalArguments(Ins, CC_HMC_Address);
...
//===----------------------------------------------------------------------===//
SDValue
HMCTargetLowering::LowerReturn(SDValue Chain,
                               CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
...
//    }
    ...
    return DAG.getNode(HMCISD::PSEUDO_RET, DL, MVT::Other, RetOps);
}

SDValue HMCTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const{
    ...
    return DAG.getNode(HMCISD::WRAPPER, SDLoc(Op),
                       getPointerTy(DLayout), Result);

}

SDValue HMCTargetLowering::LowerExternalSymbol(SDValue Op, SelectionDAG &DAG) const{
    ...
    return DAG.getNode(HMCISD::WRAPPER, dl,
                       getPointerTy(DLayout), Result);
}

SDValue
HMCTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
    // VASTART stores the address of the VarArgsFrameIndex slot into the
    // memory location argument.
    MachineFunction &MF = DAG.getMachineFunction();
    HMCFunctionInfo *QFI = MF.getInfo<HMCFunctionInfo>();
...
SDValue HMCTargetLowering::LowerConstant(SDValue Op, SelectionDAG &DAG) const {
...
//SDValue HMCTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerLOAD(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerADDSUBWithFlags(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (CinFlag) {
        ...
        return DAG.getNode(HMCISD::ADDCG, dl, VTs, lhs, rhs, Op3);
    } else {
...
// HMC does not natively support 64 bit instructions, so we should
// not worry about the upper part of the result.
SDValue HMCTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (ValueType == MVT::i32){
        ...
        return DAG.getNode(HMCISD::MPYLL, dl, ValueType, lhs, rhs);
    }
...
SDValue HMCTargetLowering::LowerMULHS(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
        ...
        if(C->getConstantIntValue()->getBitWidth() > 9) {
            SDValue v = DAG.getNode(HMCISD::MOV, dl, MVT::i32, RHS);
...
SDValue HMCTargetLowering::LowerMULHU(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerSDIV(SDValue Op, SelectionDAG &DAG) const {
    ...
    SDValue Reg0 = DAG.getRegister(HMC::Reg0, MVT::i32);
...
//                                Reg0, Reg0, CondCodeBr);
    ...
    ADDCGNodes.push_back(DAG.getNode(HMCISD::ADDCG, DL,
                                     VTList,
    ...
    DIVSNodes.push_back(DAG.getNode(HMCISD::DIVS, DL,
                                    VTList,
                                    Reg0, SlctDivisor,
    ...
    SDValue ORC = DAG.getNode(HMCISD::ORC, DL, MVT::i32,
    ...
    SDValue SH1ADDNode = DAG.getNode(HMCISD::SH1ADD, DL,
                                     MVT::i32, ORC,
                                     CmpRes);
...
SDValue HMCTargetLowering::LowerUDIV(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerSREM(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::LowerUREM(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCTargetLowering::CombineMinMaxLegacy(SDLoc DL, EVT VT, SDValue lhs, SDValue rhs,
    ...
    switch (CCOpcode) {
        case ISD::SETULE:
        case ISD::SETULT: {
            unsigned Opc = (lhs == True) ? HMCISD::MINU : HMCISD::MAXU;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETLE:
        case ISD::SETLT: {
            unsigned Opc = (lhs == True) ? HMCISD::MIN : HMCISD::MAX;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETGT:
        case ISD::SETGE: {
            unsigned Opc = (lhs == True) ? HMCISD::MAX : HMCISD::MIN;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETUGE:
        case ISD::SETUGT: {
            unsigned Opc = (lhs == True) ? HMCISD::MAXU : HMCISD::MINU;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        default:
            return SDValue();
...
SDValue HMCTargetLowering::PerformDAGCombine(SDNode *N,
...
SDValue HMCTargetLowering::LowerADDVec(SDValue Op, SelectionDAG &DAG) const {
...
bool
HMCTargetLowering::allowsMisalignedMemoryAccesses(EVT VT,
                                                  unsigned,
                                                  unsigned,
                                                  bool *Fast) const {
    if (Fast)
