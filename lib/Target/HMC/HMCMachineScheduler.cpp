//===- HMCMachineScheduler.cpp - MI Scheduler for HMC -------------===//
//
...
void HMCVLIWMachineScheduler::postprocessDAG() {
...
/// empirically.
bool HMCVLIWResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
  ...
  case HMC::ADJCALLSTACKUP:
  case HMC::ADJCALLSTACKDOWN:
    break;
  }
...
bool HMCVLIWResourceModel::reserveResources(SUnit *SU) {
...
void HMCVLIWMachineScheduler::schedule() {
  DEBUG(dbgs()
...
void ConvergingHMCVLIWScheduler::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<HMCVLIWMachineScheduler*>(dag);
  ...
  Top.ResourceModel = new HMCVLIWResourceModel(TM, DAG->getSchedModel());
  Bot.ResourceModel = new HMCVLIWResourceModel(TM, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void ConvergingHMCVLIWScheduler::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void ConvergingHMCVLIWScheduler::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool ConvergingHMCVLIWScheduler::SchedBoundary::checkHazard(SUnit *SU) {
...
void ConvergingHMCVLIWScheduler::SchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void ConvergingHMCVLIWScheduler::SchedBoundary::bumpCycle() {
...
void ConvergingHMCVLIWScheduler::SchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void ConvergingHMCVLIWScheduler::SchedBoundary::releasePending() {
...
void ConvergingHMCVLIWScheduler::SchedBoundary::removeReady(SUnit *SU) {
...
SUnit *ConvergingHMCVLIWScheduler::SchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void ConvergingHMCVLIWScheduler::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int ConvergingHMCVLIWScheduler::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
ConvergingHMCVLIWScheduler::CandResult ConvergingHMCVLIWScheduler::
...
SUnit *ConvergingHMCVLIWScheduler::pickNodeBidrectional(bool &IsTopNode) {
...
SUnit *ConvergingHMCVLIWScheduler::pickNode(bool &IsTopNode) {
...
//         I->getSUnit()->dump(DAG);
        ...
        if (SU->getInstr()->getOpcode() == HMC::CALL) {
...
/// that was just returned by pickNode(). However, HMCVLIWMachineScheduler needs
/// to update it's state based on the current cycle before MachineSchedStrategy
/// does.
void ConvergingHMCVLIWScheduler::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
