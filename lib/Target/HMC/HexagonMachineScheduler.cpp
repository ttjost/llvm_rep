...
extern cl::opt<bool> DisableHMCMISched;

#define DEBUG_TYPE "HMC-misched"

/// Platform-specific modifications to DAG.
void NewHMCVLIWMachineScheduler::postprocessDAG() {
...
/// empirically.
bool NewHMCVLIWResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
...
bool NewHMCVLIWResourceModel::reserveResources(SUnit *SU) {
...
void NewHMCVLIWMachineScheduler::schedule() {
  DEBUG(dbgs()
...
void NewHMCConvergingVLIWScheduler::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<NewHMCVLIWMachineScheduler*>(dag);
  ...
  Top.ResourceModel = new NewHMCVLIWResourceModel(STI, DAG->getSchedModel());
  Bot.ResourceModel = new NewHMCVLIWResourceModel(STI, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void NewHMCConvergingVLIWScheduler::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void NewHMCConvergingVLIWScheduler::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::checkHazard(SUnit *SU) {
...
void NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::bumpCycle() {
...
void NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::releasePending() {
...
void NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::removeReady(SUnit *SU) {
...
SUnit *NewHMCConvergingVLIWScheduler::NewHMCVLIWSchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void NewHMCConvergingVLIWScheduler::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int NewHMCConvergingVLIWScheduler::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
NewHMCConvergingVLIWScheduler::CandResult NewHMCConvergingVLIWScheduler::
...
SUnit *NewHMCConvergingVLIWScheduler::pickNodeBidrectional(bool &IsTopNode) {
  ...
  //
  // RegionCriticalPSets summarizes the pre,essure within the scheduled region and
...
SUnit *NewHMCConvergingVLIWScheduler::pickNode(bool &IsTopNode) {
...
/// that was just returned by pickNode(). However, NewHMCVLIWMachineScheduler needs
/// to update it's state based on the current cycle before MachineSchedStrategy
/// does.
void NewHMCConvergingVLIWScheduler::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
