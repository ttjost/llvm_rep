//===-- HMCAsmPrinter.cpp - HMC LLVM Assembly Printer -------------------===//
//
...
// of machine-dependent LLVM code to GAS-format HMC assembly language.
//
//===----------------------------------------------------------------------===//

#include "HMCAsmPrinter.h"
...
#define DEBUG_TYPE "HMC-asm-printer"
...
// FIXME :  Verify if this is correct for our HMC architecture

bool HMCAsmPrinter::runOnMachineFunction(MachineFunction &MF){
    Subtarget = &MF.getSubtarget<HMCSubtarget>();

    DEBUG(errs() << "\n\nHMC asm Printer\n\n");
    HMCFI = MF.getInfo<HMCFunctionInfo>();
...
void HMCAsmPrinter::EmitInstruction(const MachineInstr *MI){
...
void HMCAsmPrinter::printSavedRegsBitmask(raw_ostream &O){
    ...
    //unsigned CPURegSize = HMC::B

    unsigned i = 0, e = CSI.size();
    // Set CPU Bitmask
    for (; i != e; ++i){
        unsigned Reg = CSI[i].getReg();
        unsigned RegNum = getHMCRegisterNumbering(Reg);
        CPUBitmask |= (1 << RegNum);
    }
...
void HMCAsmPrinter::printHex32(unsigned Value, raw_ostream &O) {
  O << "0x";
  for (int i = 7; i >= 0; i--)
...
//	.set	nomacro
/// Frame Directive
void HMCAsmPrinter::emitFrameDirective(){
...
//                                StringRef(HMCInstPrinter::getRegisterName(stackReg)).lower() +
//                                "," + Twine(stackSize) + ", $" +
//                                StringRef(HMCInstPrinter::getRegisterName(returnReg)).lower());
}

const char *HMCAsmPrinter::getCurrentABIString() const {
    return "abi32";
}

// .entry caller, sp=$r0.1, rl=$l0.0, asize=80, arg()
// main:
void HMCAsmPrinter::EmitFunctionEntryLabel() {
//    if(OutStreamer.hasRawTextSupport())
...
void HMCAsmPrinter::EmitFunctionBodyStart() {
...
///
/// .endp
void HMCAsmPrinter::EmitFunctionBodyEnd() {
...
//  .previous
void HMCAsmPrinter::EmitStartOfAsmFile(Module &M){
...
void HMCAsmPrinter::EmitEndOfAsmFile(Module &M) {
...
MachineLocation HMCAsmPrinter::getDebugValueLocation(const MachineInstr *MI) const{
    //Handles frame address emit in HMCInstrInfo::emitFrameIndexDebugValue.
...
void HMCAsmPrinter::PrintDebugValueComment(const MachineInstr *MI,
                                           raw_ostream &OS) {
  // TODO: implement
...
bool HMCAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
    ...
    switch (MO.getType()) {
        ...
        case MachineOperand::MO_Register:
            O << "$" << StringRef(HMCInstPrinter::getRegisterName(MO.getReg())).lower();
            return false;
    }
...
extern "C" void LLVMInitializeHMCAsmPrinter() {
  RegisterAsmPrinter<HMCAsmPrinter> X(TheHMCTarget);
  RegisterAsmPrinter<HMCAsmPrinter> Y(TheHMCNewTarget);
}
